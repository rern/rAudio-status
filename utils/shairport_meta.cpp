// g++ -O2 shairport_meta.cpp -o shairport-meta $( pkg-config --cflags --libs libsystemd )
...
// Helper to get property from specific interface
...
static int on_property_change(sd_bus_message* m, void* userdata, sd_bus_error* ret_error) {
    const char* interface;
    sd_bus_message_read(m, "s", &interface);
    
    // Enter the changed properties dictionary
    sd_bus_message_enter_container(m, SD_BUS_TYPE_ARRAY, "{sv}");
    const char* prop_name;
    sd_bus_message_enter_container(m, SD_BUS_TYPE_DICT_ENTRY, "sv");
    sd_bus_message_read(m, "s", &prop_name);
    
    std::cout << "[Monitor Event] Property Changed: " << prop_name << std::endl;
...
// OPTION #2: Monitor Changes
void start_monitor(sd_bus* bus) {
    sd_bus_match_signal(bus, nullptr, "org.gnome.ShairportSync", 
        "/org/gnome/ShairportSync", "org.freedesktop.DBus.Properties", 
        "PropertiesChanged", on_property_change, nullptr);
    
    std::cout << "Monitoring for changes... (Ctrl+C to stop)" << std::endl;
    while (true) {
